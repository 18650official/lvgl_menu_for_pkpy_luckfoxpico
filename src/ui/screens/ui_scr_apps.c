...
#include "ui/screens/ui_scr_apps.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <dirent.h>
...
{
    const char * full_path = lv_event_get_user_data(e);
    if (!full_path) return;

    char command[512];
    snprintf(command, sizeof(command), "/oem/lv_execute/nes_start.sh \"%s\" &", full_path);
    ...
    // 注意：如果在退出时需要释放内存，可以在别处处理，这里为了保持简洁直接传递
}
...
static void stella_game_launch_event_handler(lv_event_t * e)
{
    ...
    char command[512];
    snprintf(command, sizeof(command), "/oem/lv_execute/stella_start.sh \"%s\" &", full_path);
...
// 辅助扫描指定目录并加入列表的函数
static void scan_and_add_nes_dir(lv_obj_t * parent, lv_group_t * g, const char * dir_path)
{
    ...
    while ((dir = readdir(d)) != NULL) {
        if (dir->d_type == DT_REG) {
            ...
            if (path_copy) {
                // 列表显示依然只显示文件名，视觉上整洁
                lv_obj_t * btn_game = ui_comp_create_nes_list_btn(parent, dir->d_name);
                lv_obj_add_event_cb(btn_game, nes_game_launch_event_handler, LV_EVENT_CLICKED, path_copy);
...
static void scan_and_add_stella_dir(lv_obj_t * parent, lv_group_t * g, const char * dir_path)
{
    DIR *d = opendir(dir_path);
    if (!d) return;
    ...
    while ((dir = readdir(d)) != NULL) {
        if (dir->d_type == DT_REG) {
            ...
            if (path_copy) {
                lv_obj_t * btn_game = ui_comp_create_nes_list_btn(parent, dir->d_name);
                lv_obj_add_event_cb(btn_game, stella_game_launch_event_handler, LV_EVENT_CLICKED, path_copy);
                lv_group_add_obj(g, btn_game);
            }
        }
    }
...
static void create_nes_browser_screen(lv_obj_t * parent)
{
    ...
    // 1. 扫描机身内置目录
    scan_and_add_nes_dir(nes_browser_screen, g, "/oem/nes_games");

    // 2. 检查 SD 卡并处理
    if (access("/mnt/sdcard", F_OK) == 0) {
        char sd_dir[256] = "/mnt/sdcard/nes_games";
        if (access(sd_dir, F_OK) != 0) {
            mkdir(sd_dir, 0777); // 如果不存在则自动创建
        }
        scan_and_add_nes_dir(nes_browser_screen, g, sd_dir);
    }
...
static void create_stella_browser_screen(lv_obj_t * parent)
{
    ...
    // 1. 扫描机身内置目录
    scan_and_add_stella_dir(stella_browser_screen, g, "/oem/atari_games");

    // 2. 检查 SD 卡并处理
    if (access("/mnt/sdcard", F_OK) == 0) {
        char sd_dir[256] = "/mnt/sdcard/atari_games";
        if (access(sd_dir, F_OK) != 0) {
            mkdir(sd_dir, 0777);
        }
        scan_and_add_stella_dir(stella_browser_screen, g, sd_dir);
    }
...
void ui_open_music_screen(void)
{
    ...
    lv_timer_handler();
    usleep(16000);
    system("/oem/lv_execute/music_start_all.sh < /dev/null &");
}